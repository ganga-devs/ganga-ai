...
"""
This class is responsible for working with the terminal interface and state.
It also is responsible for working with the backend.
"""


class Terminal:
    errString = "Processing the error to help you..."
    ...
    def _build_command_history_for_assists(self, maxCount: int = 5) -> str:

        # Need the session id to be able to extract history of the current session to feed it as context to the backend
        ...
        starting_count = max(current_execution_count - maxCount, 1)
        context: str = (
    ...
    def _handle_initial_error(self, err_value: str) -> None:
        self._display_formatted_output(self.errString)

...
class Terminal:
    ...
    def _build_context_for_assists(self, count: int = 5) -> str:
        ...
        current_ipython_session_id: int = self.ipython_history_manager.get_last_session_id()
        ...
        for _, _, (input_text, output_text) in self.ipython_history_manager.get_range(session=current_ipython_session_id, start=starting_count, stop=current_execution_count, output=True):
